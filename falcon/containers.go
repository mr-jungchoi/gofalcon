...
func FalconImageURIWithCSRegistryEnv(falconCloud CloudType, sensorType SensorType, csRegistryEnv CSRegistryEnv) string {
	...
	case Snapshot:
		return fmt.Sprintf("%s/falcon-snapshot/%s/%s/cs-snapshotscanner", registryFQDN(falconCloud), registryCloud(falconCloud), csRegistryEnv)
	case FCSCli:
		return fmt.Sprintf("%s/fcs/%s/%s/cs-fcs", registryFQDN(falconCloud), registryCloud(falconCloud), csRegistryEnv)
	case SHRAController:
		return fmt.Sprintf("%s/falcon-selfhostedregistryassessment/%s/falcon-jobcontroller", registryFQDN(falconCloud), csRegistryEnv)
	case SHRAExecutor:
		return fmt.Sprintf("%s/falcon-selfhostedregistryassessment/%s/falcon-registryassessmentexecutor", registryFQDN(falconCloud), csRegistryEnv)
	default:
		return fmt.Sprintf("%s/falcon-sensor/%s/%s/falcon-sensor", registryFQDN(falconCloud), registryCloud(falconCloud), csRegistryEnv)
	}
}
